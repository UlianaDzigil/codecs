  ...
  implicit def listDecoder[A](implicit decoder: Decoder[A]): Decoder[List[A]] =
    Decoder.fromPartialFunction{
      case Json.Arr(x) => {
        ...
        if (items.forall(_.isDefined)) items.map(_.get)
        else Option.empty.get
      }
      case _ => Option.empty.get
    }
...
case class Password(value: String, code: List[Int])
...
trait PasswordCodecs {
  ...
  implicit val passwordEncoder: Encoder[Password] =
    ObjectEncoder.field[String]("value")
      .zip(ObjectEncoder.field[List[Int]]("code"))
      .transform[Password](password => (password.value, password.code))

  implicit val passwordDecoder: Decoder[Password] =
    Decoder.field[String]("value")
      .zip(Decoder.field[List[Int]]("code"))
      .transform[Password] {
        case (value, code) => Password(value, code)}
