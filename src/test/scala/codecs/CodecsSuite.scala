...
class CodecsSuite
  ...
  @Test def `it is possible to encode and decode password`(): Unit = {
    checkProperty(Prop.forAll((value: String, code: List[Int]) => encodeAndThenDecodeProp(Password(value, code))))
  }
